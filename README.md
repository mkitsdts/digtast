...
`digital-labor` 是一个强大的数字助理容器服务。它既可以作为独立的本地服务运行，也可以接入核心中枢（Center）在云端协同工作。
...
1. **网关 (Gateway)**：解决与主节点的通信传输，并负责管理视觉显示（如启动 VNC 远程桌面传输）。
...
```bash
# 启动 gRPC 服务，默认监听 50051 端口
go run main.go
```

启动成功后，控制台会输出如下日志，表示服务正在等待请求：
```text
time=... level=INFO msg="server listening at" address=[::]:50051
```
